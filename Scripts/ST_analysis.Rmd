...
R objects are processed using VisiumDataQC_v1.3_cleaned.R file. Mitochondrial, ribosomal, and hemoglobin percents are calculated in the QC step.
...
qupath <- read.csv("/Users/tili/Desktop/CIIR/results/Cell_annotation_qupath_analyzed/cell_qupath_combined_20250509.csv")
...
anno <- bcsa1_freq %>% dplyr::select(integrated_clusters, spot_density) %>% unique()

# Plot the cell type percent in each tumor cluster
ggplot() + 
  geom_bar(bcsa1_freq, mapping = aes(x = integrated_clusters, y = Percent * 100, fill = Classification),
           stat = "identity", position = "stack") + theme_bw() +
  ylab("Percent (%)") +
  scale_fill_manual(values = c("Tumor" = "#FF9D99", 
  ...
  theme(panel.grid.major = element_blank(),   
        ...
        plot.title = element_text(size = 18),
        axis.title = element_text(size = 15)) +
  geom_text(data = anno, mapping = aes(x = integrated_clusters, y = spot_density, label = spot_density), 
            vjust = -10, 
            color = "black", size = 5)

```

Scripts to perform Spatial inferCNV analysis are available in the folder Spatial_inferCNV. The scaled infercnv object is loaded for visualization using the section below.

## Plot BCSA1 inferCNV results

```{r BCSA1 inferCNV}

cnv <- readRDS("/Users/tili/Desktop/CIIR/results/infercnv/BCSA1_i3_90/run.final.scaled.infercnv_obj")
...
# Save results
write.csv(bcsa1_cnv_matrix, file = "/Users/tili/Desktop/CIIR/results/infercnv/BCSA1_i3_90/bcsa1_cnv_matrix.csv")
write.csv(gene_order, file = "/Users/tili/Desktop/CIIR/results/infercnv/BCSA1_i3_90/gene_order.csv")

bcsa1_cnv_matrix <- readr::read_csv("/Users/tili/Desktop/CIIR/results/inferCNV/BCSA1_i3_50/bcsa1_cnv_matrix.csv")
gene_order <- read.csv("/Users/tili/Desktop/CIIR/results/inferCNV/BCSA1_i3_50/gene_order.csv")

all(bcsa1_cnv_matrix[,1] == gene_order$gene)
rownames(gene_order) <- gene_order$gene
bcsa1_cnv_matrix <- as.data.frame(bcsa1_cnv_matrix)
rownames(bcsa1_cnv_matrix) <- bcsa1_cnv_matrix[,1]

bcsa1_cnv_matrix <- bcsa1_cnv_matrix[,-1]
gene_order <- gene_order[,-1]
gene_order$chr <- factor(gene_order$chr, levels = unique(gene_order$chr))
gene_order$gene <- factor(gene_order$gene, levels = unique(gene_order$gene))
...
tumor <- read.delim("/Users/tili/Desktop/CIIR/results/infercnv/BCSA1_filtered_tumor_50_percent_spots_rep1_for_inferCNV.txt", header = F)
...
temp1 <- cutree(dend1, h = 153) %>% as.data.frame() # 50% purity
temp1 <- cutree(dend1, h = 132) %>% as.data.frame() # 70% purity
temp1 <- cutree(dend1, h = 110) %>% as.data.frame() # 90% purity
temp1$cluster <- ifelse(str_detect(temp1$., "1"), "A", 
                        ifelse(str_detect(temp1$., "2"), "B",
                               ifelse(str_detect(temp1$., "3"), "C", "D")))
table(temp1)
clone1 <- temp1$cluster
names(clone1) <- rownames(temp1)

dend2 <- col_order_list[[2]]
temp1 <- cutree(dend2, h = 153) %>% as.data.frame() # 50% purity
temp1 <- cutree(dend2, h = 132) %>% as.data.frame() # 70% purity
temp1 <- cutree(dend2, h = 110) %>% as.data.frame() # 90% purity
temp1$cluster <- ifelse(str_detect(temp1$., "1"), "B", "C")
...
SpatialDimPlot(bcsa, group.by = "inferCNV", cols = co) + patchwork::plot_layout(guides = "collect")
...
# List oncogenes
onco <- read.delim("/Users/tili/Desktop/CIIR/results/inferCNV/Genes_inOncoKB_BCSA1_i3.txt")
onco <- onco %>% dplyr::filter(Onco == "Yes")
oncogene <- onco[which(onco$symbol %in% colnames(t(tumor_matrix))),]
oncogene <- oncogene %>% dplyr::filter(chr %in% c("chr9", "chr12", "chr16", "chr18"))
oncogene$chr <- factor(oncogene$chr, levels = c("chr9", "chr12", "chr16", "chr18"))
oncogene <- oncogene %>% dplyr::arrange(chr)
t_matrix <- t(tumor_matrix)
col_ha <- HeatmapAnnotation(foo = anno_mark(at = which(colnames(t_matrix) %in% c("NOTCH1",
                                                                                 "BCL2", "CDH2", "PIK3C3")),
                                            labels = c("NOTCH1",
                                                       "BCL2", "CDH2", "PIK3C3")))


pdf(file = "/Users/tili/Desktop/CIIR/results/inferCNV/BCSA1_inferCNV_heatmap_90_percent_annotated_20250514.pdf", width = 18, height = 9)
Heatmap(as.matrix(t_matrix), cluster_rows = T, 
...
Processed R objects are obtained using VisiumDataAnalysisPAM50_v1.4_cleaned.R file. Spatial PAM50 calculation is performed using the raw count matrix. Post-filtering step is performed in the following section using epithelial cells > 20% as cutoff.
...
ggplot(cluster_freq, aes(x = PAM50_aims, y = Percent * 100, fill = integrated_clusters)) + 
  geom_bar(stat = "identity", position = "stack") + theme_bw() + ylab("Percent (%)") +
...
# Plot the cell type percent in each tumor cluster
ggplot(bcsa1_freq, aes(x = PAM50_aims, y = Percent * 100, fill = Classification)) + 
  geom_bar(stat = "identity", position = "stack") + theme_bw() +
  scale_fill_manual(values = c("Tumor" = "#FF9D99", 
          "Immune cells" = "#FFDE18", "Stroma" = "#9BCCE3")) + xlab("") + ylab("Percent (%)") +
  theme(panel.grid.major = element_blank(),   
        panel.grid.minor = element_blank(),
        axis.text = element_text(size = 14, colour = "black"),
        legend.title = element_text(size = 14),    
...
anno <- bcsa2_freq %>% dplyr::select(integrated_clusters, spot_density) %>% unique()

# Plot the cell type percent in each tumor cluster
ggplot() + 
  geom_bar(bcsa2_freq, mapping = aes(x = integrated_clusters, y = Percent * 100, fill = Classification),
           stat = "identity", position = "stack") + theme_bw() +
  scale_fill_manual(values = c("Tumor" = "#FF9D99", 
          "Immune cells" = "#FFDE18", "Stroma" = "#9BCCE3")) + xlab("") + ylab("Percent (%)") +
  theme(panel.grid.major = element_blank(),   
...
cnv <- readRDS("/Users/tili/Desktop/CIIR/results/infercnv/BCSA2_i3_50/run.final.scaled.infercnv_obj")
...
# Save results
write.csv(bcsa2_cnv_matrix, file = "/Users/tili/Desktop/CIIR/results/infercnv/BCSA2_i3_50/bcsa2_cnv_matrix.csv")
write.csv(gene_order, file = "/Users/tili/Desktop/CIIR/results/infercnv/BCSA2_i3_50/gene_order.csv")

bcsa2_cnv_matrix <- readr::read_csv("/Users/tili/Desktop/CIIR/results/inferCNV/BCSA2_i3_50/bcsa2_cnv_matrix.csv")
gene_order <- read.csv("/Users/tili/Desktop/CIIR/results/inferCNV/BCSA2_i3_50/gene_order.csv")

all(bcsa2_cnv_matrix[,1] == gene_order$gene)
rownames(gene_order) <- gene_order$gene
bcsa2_cnv_matrix <- as.data.frame(bcsa2_cnv_matrix)
rownames(bcsa2_cnv_matrix) <- bcsa2_cnv_matrix[,1]

bcsa2_cnv_matrix <- bcsa2_cnv_matrix[,-1]
gene_order <- gene_order[,-1]
gene_order$chr <- factor(gene_order$chr, levels = unique(gene_order$chr))
gene_order$gene <- factor(gene_order$gene, levels = unique(gene_order$gene))
...
normal$type <- rep("Normal", nrow(normal))
tumor <- read.delim("/Users/tili/Desktop/CIIR/results/infercnv/BCSA2_filtered_tumor_50_percent_spots_rep1_for_inferCNV.txt", header = F)
tumor$type <- rep("Tumor", nrow(tumor))
spots <- rbind(normal, tumor)
...
# Cut tree at height 150 to get 3 clusters
dend1 <- col_order_list[[1]]
temp1 <- cutree(dend1, h = 1594) %>% as.data.frame() # 50% purity
temp1 <- cutree(dend1, h = 1380) %>% as.data.frame() # 70% purity
temp1 <- cutree(dend1, h = 1036) %>% as.data.frame() # 90% purity
table(temp1)
...
dend2 <- col_order_list[[2]]
temp1 <- cutree(dend2, h = 1594) %>% as.data.frame() # 50% purity
temp1 <- cutree(dend2, h = 1380) %>% as.data.frame() # 70% purity
temp1 <- cutree(dend2, h = 1036) %>% as.data.frame() # 90% purity
table(temp1)
temp1$cluster <- dplyr::recode(temp1$., `1` = "A", `2` = "C", `3` = "D")#
                               #, `4` = "F", `5` = "G", `6` = "H")
clone2 <- temp1$cluster
names(clone2) <- rownames(temp1)
...
bcsa <- subset(bcsa, cells = names(clone))
bcsa <- AddMetaData(bcsa, clone, col.name = "inferCNV")
co <- c("A" = "#35A153", "B" = "#FFCD44", "C" = "#1d9bf7")
SpatialDimPlot(bcsa, group.by = "inferCNV", cols = co, ncol = 2) + patchwork::plot_layout(guides = "collect")
...
# List oncogenes
onco <- read.delim("/Users/tili/Desktop/CIIR/results/inferCNV/Genes_inOncoKB_BCSA2_i3.txt")
onco <- onco %>% dplyr::filter(Onco == "Yes")
oncogene <- onco[which(onco$symbol %in% colnames(t(tumor_matrix))),]
oncogene <- oncogene %>% dplyr::filter(chr %in% c("chr9", "chr15", "chr17"))
oncogene$chr <- factor(oncogene$chr, levels = c("chr9", "chr15", "chr17"))
oncogene <- oncogene %>% dplyr::arrange(chr)
...
pdf(file = "/Users/tili/Desktop/CIIR/results/inferCNV/BCSA2_inferCNV_heatmap_1_clone_50_percent_annotated_20250516.pdf", width = 18, height = 9)
Heatmap(as.matrix(t_matrix), cluster_rows = T, 
        show_row_names = F, show_column_names = F, cluster_columns = F,
        col = col_fun, 
        row_split = clones$clone,
...
# Plot the cell type percent in each tumor cluster
ggplot(bcsa2_freq, aes(x = PAM50_aims, y = Percent * 100, fill = Classification)) + 
  geom_bar(stat = "identity", position = "stack") + theme_bw() +
  scale_fill_manual(values = c("Tumor" = "#FF9D99", 
          "Immune cells" = "#FFDE18", "Stroma" = "#9BCCE3")) + xlab("")  + ylab("Percent (%)") +
  theme(panel.grid.major = element_blank(),   
...
anno <- bcsa3_freq %>% dplyr::select(integrated_clusters, spot_density) %>% unique()

# Plot the cell type percent in each tumor cluster
ggplot() + 
  geom_bar(bcsa3_freq, mapping = aes(x = integrated_clusters, y = Percent * 100, fill = Classification),
           stat = "identity", position = "stack") + theme_bw() +
  scale_fill_manual(values = c("Tumor" = "#FF9D99", 
          "Immune cells" = "#FFDE18", "Stroma" = "#9BCCE3")) + xlab("") + ylab("Percent (%)") +
  theme(panel.grid.major = element_blank(),   
...
cnv <- readRDS("/Users/tili/Desktop/CIIR/results/infercnv/BCSA3_i3_90/run.final.scaled.infercnv_obj")
...
# Save results
write.csv(bcsa3_cnv_matrix, file = "/Users/tili/Desktop/CIIR/results/infercnv/BCSA3_i3_90/bcsa3_cnv_matrix.csv")
write.csv(gene_order, file = "/Users/tili/Desktop/CIIR/results/infercnv/BCSA3_i3_90/gene_order.csv")

bcsa3_cnv_matrix <- readr::read_csv("/Users/tili/Desktop/CIIR/results/inferCNV/BCSA3_i3_90/bcsa3_cnv_matrix.csv")
gene_order <- read.csv("/Users/tili/Desktop/CIIR/results/inferCNV/BCSA3_i3_90/gene_order.csv")

all(bcsa3_cnv_matrix[,1] == gene_order$gene)
rownames(gene_order) <- gene_order$gene
bcsa3_cnv_matrix <- as.data.frame(bcsa3_cnv_matrix)
rownames(bcsa3_cnv_matrix) <- bcsa3_cnv_matrix[,1]

bcsa3_cnv_matrix <- bcsa3_cnv_matrix[,-1]
gene_order <- gene_order[,-1]
gene_order$chr <- factor(gene_order$chr, levels = unique(gene_order$chr))
gene_order$gene <- factor(gene_order$gene, levels = unique(gene_order$gene))
...
normal$type <- rep("Normal", nrow(normal))
tumor <- read.delim("/Users/tili/Desktop/CIIR/results/infercnv/BCSA3_filtered_tumor_90_percent_spots_rep1_for_inferCNV.txt", header = F)
tumor$type <- rep("Tumor", nrow(tumor))
spots <- rbind(normal, tumor)
...
# Cut tree at height 150 to get 3 clusters
dend1 <- col_order_list[[1]]
temp1 <- cutree(dend1, h = 255) %>% as.data.frame() # 50% purity
temp1 <- cutree(dend1, h = 297) %>% as.data.frame() # 70% purity
temp1 <- cutree(dend1, h = 176) %>% as.data.frame() # 90% purity
table(temp1)
temp1$cluster <- ifelse(str_detect(temp1$., "1"), "A", 
                        ifelse(str_detect(temp1$., "2"), "B", "C"))
clone1 <- temp1$cluster
names(clone1) <- rownames(temp1)

dend2 <- col_order_list[[2]]
# 50% and 70% purity
temp1 <- cutree(dend2, h = 255) %>% as.data.frame() # 50% purity
temp1 <- cutree(dend2, h = 297) %>% as.data.frame() # 70% purity
table(temp1)
temp1$cluster <- dplyr::recode(temp1$., `1` = "B", `2` = "C", `3` = "D") # 50% and 70% purity

# 90% purity
temp1 <- cutree(dend2, h = 176) %>% as.data.frame() # 90% purity
table(temp1)
temp1$cluster <- dplyr::recode(temp1$., `1` = "C") # 90% purity
...
# List oncogenes
onco <- read.delim("/Users/tili/Desktop/CIIR/results/inferCNV/Genes_inOncoKB_BCSA3_i3.txt")
onco <- onco %>% dplyr::filter(Onco == "Yes")
oncogene <- onco[which(onco$symbol %in% colnames(t(tumor_matrix))),]
oncogene <- oncogene %>% dplyr::filter(chr %in% c("chr7", "chr8", "chr17"))
oncogene$chr <- factor(oncogene$chr, levels = c("chr7", "chr8", "chr17"))
oncogene <- oncogene %>% dplyr::arrange(chr)

t_matrix <- t(tumor_matrix)

col_ha <- HeatmapAnnotation(foo = anno_mark(at = which(colnames(t_matrix) %in% c("MYC", "ERBB2")),
                                            labels = c("MYC", "ERBB2")))

pdf(file = "/Users/tili/Desktop/CIIR/results/inferCNV/BCSA3_inferCNV_heatmap_3_clones_90_percent_annotated_20250514.pdf", width = 18, height = 9)
Heatmap(as.matrix(t_matrix), cluster_rows = T, 
        show_row_names = F, show_column_names = F, cluster_columns = F,
        col = col_fun, 
        row_split = clones$clone,
...
# Plot the cell type percent in each tumor cluster
ggplot(bcsa3_freq, aes(x = PAM50_aims, y = Percent * 100, fill = Classification)) + 
  geom_bar(stat = "identity", position = "stack") + theme_bw() + ylab("Percent (%)") +
  scale_fill_manual(values = c("Tumor" = "#FF9D99", 
          "Immune cells" = "#FFDE18", "Stroma" = "#9BCCE3")) + xlab("") +
  theme(panel.grid.major = element_blank(),   
